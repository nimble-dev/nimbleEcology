...
Some distribution names are necessarily followed by a suffix declaring the type of parameters, for example the `_s` in `dOcc_s`. NIMBLE uses a static typing system, meaning that a function must know in advance if a certain argument will be a scalar, vector, or matrix. There may be notation such as `sv` or `svm` if there are two or three parameters that can be time-independent (`s`) or time-dependent (`v`, `m`) in one or more dimensions. In general, `s` corresponds to a scalar argument, `v` to a vector argument, and `m` to a matrix argument. The order of these type indicators will correspond to the order of the parameters needing to be specified, but always check the documentation when using a new distribution with the R function `?` (both `?dOcc` and `?dOcc_s` work).
...
`y[i, 1:T] ~ dDHMM(init = initial_probs[i, 1:T], obsProb = p[1:nStates, 1:nCat], transProb = Trans[i, 1:nStates, 1:nStates, 1:T], len = T)`
...
- `obsProb[i, j]` (i.e., `p[i, j]` in this example) is probability of observing an individual who is truly in state `i` as being in state `j`.  This is $p_{i, j}$ above if indexing by $t$ is not needed.  If observation probabilities are time-dependent (in `dHMMo` and `dDHMMo`), then `obsProb[i, j, t]` is $p_{i, j, t}$ above.
...
An occupancy model gives the probability of a series of detection/non-detection records for a species during multiple visits to each of multiple site.  The occupancy distributions in `nimbleEcology` give the probability of the detection history for one site, so this summary focuses on data from one site.  These distributions can be applied to data from multiple sites, for example using a `for` loop.
...
- `dDynOcc_ss`: Both are time-independent.
- `dDynOcc_sv`: Persistence is time-independent (scalar) and colonization is time-independent (vector).
- `dDynOcc_vs`: Persistence is time-dependent (vector) and colonization is time-independent (scalar).
- `dDynOcc_vv`: Both are time-dependent.

An example for `dDynOcc_sv` is:

`y[i, 1:T] ~ dDynOcc_sv(init = psi1[i], probPersist = phi[i], probColonize = gamma[i, 1:T], len = T)`
