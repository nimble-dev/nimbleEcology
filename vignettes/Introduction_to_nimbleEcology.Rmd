...
Cormack-Jolly-Seber models give the probability of a capture history for an individual, conditional on first capture, based on parameters for survival and detection probability.  `nimbleEcology` provides variants for time-independent and time-dependent survival and/or detection probability.  Of course, the survival and detection parameters for the CJS probability may themselves depend on other parameters and/or random effects.
...
- The $t^{\mbox{th}}$ element of `probCapture` is $\phi_t$ above, namely the probability of capture, if alive, at time $t$.
...
In a HMM, "dead" is simply another state, and "unobserved" is a possible observation.  Thus, HMMs are generalzation of the CJS models. In capture-recapture, HMMs encompass multi-state and multi-event models.

We again use the factorization
\[
P(\mathbf{y} | \mathbf{\phi}, \mathbf{p}) = \prod_{t = 1}^T P(y_{t} | y_{1:t-1}, \mathbf{\phi}, \mathbf{p})
\]
In the case of an HMM, $y_t$ is the recorded state at time $t$, taking an integer value, with the possibility that one possible value corresponds to "unobserved".

Define $A_{t,i}$ as the probability that the individual is in state $i$ at time $t$, given $y_{1:t-1}$, the data up to the previous time.  Define $p_{t,i,j}$ as the probability that, at time $t$, an individual in state $i$ is observed in state $j$.  Then $P(y_{t} | y_{1:t-1}, \mathbf{\phi}, \mathbf{p})$ is calculated as:
\[
P(y_{t} | y_{1:t-1}, \mathbf{\phi}, \mathbf{p}) = \sum_i A_{t, i} p_{t, i, y_t}
\]
where $j = y_t$ is the observed state of the individual.

Define $G_{t, i}$ as the probability that the individual is in state $i$ at time $t$ given $y_{1:t}$, the data up to the current time.  Define $\phi_{t, i, j}$ as the probability that, from time $t-1$ to $t$, an individual in state $i$ transitions to state $j$.  Then the calculation of $A_{t, j}$ for each possible state $j$ is given by:
\[
A_{t, j} = \sum_i G_{t-1, i} \phi_{t, i, j}
\]

Finally, calculation of $G_{t, i}$ for each possible state $i$ is done by conditioning on $y_t$ as follows:
\[
G_{t, i} = \frac{A_{t, i} p_{t, i, y_t}}{P(y_{t} | y_{1:t-1}, \mathbf{\phi}, \mathbf{p})}
\]

The calculation of the total probability of a detection history with uncertain state information starts by initializing $A_{1, i}$ to some choice of initial (or prior) probability of being in each state before any observations are made.  Then, starting with $t = 1$, we calculate $P(y_{t} | y_{1:t-1}, \mathbf{\phi}, \mathbf{p})$.  If $t < T$, we prepare for time step $t+1$ by calculating $G_{t, i}$ for each $i$ followed by $A_{t+1, i}$ for each $i$.

If the transition probabilities, $\phi_{t, i, j}$, actually depend on $t$, we refer to the model as "Dynamic", namely a DHMM instead of an HMM.  It may also be the case that observation probabilities $p_{t, i, j}$ depend on $t$ or not.
...
`y[i, 1:T] ~ dDHMM(init = initial_probs[i, 1:T], Z = obsProbs[1:nStates, 1:nStates], T = transProbs[i, 1:nStates, 1:nStates, 1:T], len = T)`

Note the following points:

- As above, this is written as if `i` indexes individuals in the model, but this is arbitrary as an example.
- `init[i]` is the initial probability of being in state `i`, namely `A_{0, i}` above.
- `Z[i, j]` (I decided to wait to see if we rename these.)
- `T[i, j, t]`
- `len`

## Occupancy

An occupancy model gives the probability of a series of detection/non-detection records for a species during multiple visits to one site.
